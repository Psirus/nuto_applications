...
def SetConstitutiveLaw(structure):
    additive_input_id = structure.ConstitutiveLawCreate(
            nuto.eConstitutiveType_ADDITIVE_INPUT_EXPLICIT)
    additive_output_id = structure.ConstitutiveLawCreate(
            nuto.eConstitutiveType_ADDITIVE_OUTPUT)

    damage_id = structure.ConstitutiveLawCreate(
            nuto.eConstitutiveType_GRADIENT_DAMAGE_ENGINEERING_STRESS)
    structure.ConstitutiveLawSetParameterDouble(damage_id,
            nuto.eConstitutiveParameter_YOUNGS_MODULUS, 25e3)
    structure.ConstitutiveLawSetParameterDouble(damage_id,
            nuto.eConstitutiveParameter_POISSONS_RATIO, .2)
    structure.ConstitutiveLawSetParameterDouble(damage_id,
            nuto.eConstitutiveParameter_NONLOCAL_RADIUS, 1.3)
    structure.ConstitutiveLawSetParameterDouble(damage_id,
            nuto.eConstitutiveParameter_NONLOCAL_RADIUS_PARAMETER, 0.)
    structure.ConstitutiveLawSetParameterDouble(damage_id,
            nuto.eConstitutiveParameter_TENSILE_STRENGTH, 4.)
    structure.ConstitutiveLawSetParameterDouble(damage_id,
            nuto.eConstitutiveParameter_COMPRESSIVE_STRENGTH, 4. * 10)
    structure.ConstitutiveLawSetParameterDouble(damage_id,
            nuto.eConstitutiveParameter_FRACTURE_ENERGY, 0.021)
#
